                ...
                <div class="row">
                    ...
                    <div class=" mb-4">
                        ...
                        <select name="category" id="category" class="form-control form-select border-0  @error('category')  is-invalid @enderror">
                            @foreach(\App\Models\Category::all() as $category)
                                <option class="" value="{{ $category->id }}" {{$category->id == old('$category')?'selected':''}}  > {{ $category->title }} </option>
                            @endforeach
                        </select>
                        @error('category')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="form-floating mb-3">
                        <textarea name="description"
                                  ...
                                  placeholder="Leave a comment here" id="floatingTextarea2" style="height: 250px">{{ old('description' , $post->description ) }}</textarea>
